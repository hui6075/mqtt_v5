...
### 3.3.1 PUBLISH 固定报头 Fixed Header
...
PUBLISH报文的2个QoS比特位不能同时设置为1 \[MQTT-3.3.1-4\]。如果服务端或客户端收到QoS 2个比特位都为1的无效PUBLISH报文，使用包含原因码为0x81（无效报文）的DISCONNECT报文关闭网络连接，如4.13节所述。
...
服务端不能延迟发送任何报文，除了PUBLISH报文--如果已发送且没有收到确认的PUBLISH报文数量已到达客户端的接收最大值（Receive Maximum） \[MQTT-3.3.4-10\]。
